...
- **Possibly neither**: looks decorative, redundant with a Mermaid diagram already earned from the same material, or off-subject. This is never a silent drop (see Decide below); it is a `(? ...)` candidate for exclusion, not a completed decision.
...
**Decide.** There is no worthiness test to pass: the image is real, supplied material, not something that has to earn its existence the way a generated diagram does, and the user attached it for a reason. The only real question is placement: does some section of the doc actually walk through what the image shows? When one does, embed it there; that is the whole decision, made silently, because there is nothing to confirm.

**Excluding a supplied image is never silent.** Whether the reason is "no section discusses it", "looks decorative", "redundant with a diagram already drawn", or anything else, do not drop the image and only mention it in Sources afterward. Interactively, raise it as its own `(? drop <image>: <reason>)` item in the outline and ask; the recommended option is always to keep it (folded into the closest-fitting section, or given a small new one) rather than to leave it out, since a reader who never sees the doc's draft cannot object to material that silently never arrived. Headless or on escape, apply that same bias instead of asking: embed it in the section it is closest to, even loosely, and record the placement as uncertain in the Assumptions block. An image is left out entirely only on the user's own explicit word, and even then it gets a `not used: <the user's stated reason>` line under Sources, not a bare `not used`.
